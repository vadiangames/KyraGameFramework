	...
	void run() {
		...
		if(m_Window.create(windowSettings)) {
			...
			if(m_Renderer.create(m_Window)) {
				...
				kyra::IIndexBuffer::Ptr indexBuffer = m_Renderer.createIndexBuffer( );
					indexBuffer->create(indices.size(), sizeof(indices), &indices[0], kyra::BufferType::STATIC_DRAW);
				...
				while(m_Window.isOpen()) {
					...
					m_Renderer.clear();
						m_Renderer.draw(vertexBuffer, indexBuffer, program, vertexLayout);
					m_Renderer.display();
				}
